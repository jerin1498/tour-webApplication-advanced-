...
const createSendToken = (user, statusCode, req, res) => {
    ...
    res.cookie('jwt',
        token,
        {
            ...
            secure: req.secure || req.headers['x-forwarded-proto'] === 'https'// only send the cookie if the request is https, not for development req.secure is a built in node function to check wheather its a https connection and req.headers['x-forwarded-proto'] === 'https' is only specfic to heroku deployment platform fo if you host your application in another webhosting services no need to use req.headers['x-forwarded-proto'] === 'https' ths
        }
    );
