...
const bookingController = require('./controllers/bookingController');// for stripe payment
...
app.enable('trust proxy'); // deployment

app.use(cors()); // to allow share out api to another web applications

app.options('*', cors()); // to allow non simple request example put, patch , delete ect...
// template engine
...
app.post('/webhook-checkout',
    ...
    bookingController.webhookCheckout); // only work before the json parser because the stripe webhook only accept data in binary streams if the data pass trough the json parser then the data converted into json data so this must be placed above the json parser inorder to work
